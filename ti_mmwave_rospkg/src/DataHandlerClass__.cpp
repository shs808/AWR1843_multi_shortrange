        ...
        case READ_OBJ_STRUCT:
            ...
            while( i < mmwData.numObjOut ) {
                if (((mmwData.header.version >> 24) & 0xFF) < 3) { // SDK version is older than 3.x
                    ...
                    RScan->points[i].x = -temp[0];   // ROS standard coordinate system X-axis is forward which is the mmWave sensor Y-axis
                    RScan->points[i].y = -temp[2];  // ROS standard coordinate system Y-axis is left which is the mmWave sensor -(X-axis)
                    RScan->points[i].z = -temp[1];   // ROS standard coordinate system Z-axis is up which is the same as mmWave sensor Z-axis
                    ...
                    // Map mmWave sensor coordinates to ROS coordinate system
                    RScan->points[i].x = -mmwData.newObjOut.x;   // ROS standard coordinate system X-axis is forward which is the mmWave sensor Y-axis
                    RScan->points[i].y = -mmwData.newObjOut.z;  // ROS standard coordinate system Y-axis is left which is the mmWave sensor -(X-axis)
                    RScan->points[i].z = -mmwData.newObjOut.y;   // ROS standard coordinate system Z-axis is up which is the same as mmWave sensor Z-axis

                    radarscan.header.frame_id = frameID;
                    radarscan.header.stamp = ros::Time::now();

                    radarscan.point_id = i;
                    radarscan.x = -mmwData.newObjOut.x;
                    radarscan.y = -mmwData.newObjOut.z;
                    radarscan.z = -mmwData.newObjOut.y;
                    // radarscan.range = temp[4];
